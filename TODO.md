...
- [ ] Revue des styles de focus (`outline`, `ring`) sur tous les éléments interactifs (boutons, cartes de snowboard, hotspots, slider).
- [ ] Support clavier complet : navigation par `Tab` et déclenchement par `Space`/`Enter` sur les hotspots interactifs et le slider chromatique.
- [ ] Ajout d'attributs `aria-*` descriptifs pour les lecteurs d'écran.

## ⚡ Phase 4 : SEO & Performance
- [ ] Validation de la hiérarchie sémantique HTML5 (un seul `<h1>`, des `<section>` bien étiquetées).
- [ ] Vérification des données structurées JSON-LD générées par le composant `SEO.tsx`.
- [ ] Lancement du build de production et validation de l'absence d'erreurs TypeScript.
...
- [ ] Concevoir et générer les 3 visuels de riders premium de nuit via l'IA (`rider_lucas`, `rider_ines`, `rider_theo`).
- [ ] Restructurer `Section06Riders.tsx` avec un parent de hauteur de scroll (`h-[200vh]`) et un conteneur `sticky top-0 h-screen`.
- [ ] Connecter le défilement horizontal (`x`) des cartes au scroll vertical via `useScroll` et `useTransform` de Framer Motion.
- [ ] Créer le design des cartes avec HUD (specs techniques, lumens, planches utilisées, portrait en fond).
- [ ] Gérer l'adaptation mobile fluide (les cartes se lisent verticalement ou glissent proprement au toucher).
- [ ] Valider la compilation complète avec `npm run build`.
