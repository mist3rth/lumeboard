...
export default function Section01Problem() {
  ...
  useGSAP(() => {
    if (isMobile) {
      // Nettoyer tous les styles appliqués par GSAP sur mobile
    ...
    // Set initial state for desktop so it's ready for scrolltrigger
    ...
    const ctx = gsap.context(() => {
      ...
      // 0.0 -> 0.25 : Fade out of desktop section 1 intro content
      tl.to(contentLayerRef.current, { opacity: 0, ease: "none", duration: 0.25 }, 0);

      // 0.05 -> 0.30 : "NEW RULES" text animation
      ...
      // 0.30 -> 0.55 : Text scales up and fades out
      tl.to(textLayerRef.current, { 
        scale: 12, 
        opacity: 0, 
        ease: "power2.in",
  ...
  return (
    <div 
      ref={containerRef} 
      className="relative min-h-screen lg:h-[250vh] w-full bg-black flex flex-col" 
      id="problem-wrapper"
    >
      ...
      <div className="relative lg:sticky lg:top-0 lg:h-screen w-full lg:overflow-hidden flex flex-col justify-center py-20 lg:py-0 flex-grow">
        ...
        >
              ...
              {paragraphs.map((line, idx) => (
                <div key={idx}>
                  <motion.p
                    ...
                    animate={isMobile ? { opacity: 1, y: 0 } : (isHeadingInView ? { opacity: 1, y: 0 } : { opacity: 0, y: 15 })}
                    transition={{
                      duration: 0.8,
                      delay: idx * 0.12,
                      ease: [0.16, 1, 0.3, 1]
          ...
          <div className="lg:col-span-6 w-full flex justify-center items-center">
            <motion.div
              ...
              animate={isMobile ? { opacity: 1, scale: 1 } : (isHeadingInView ? { opacity: 1, scale: 1 } : { opacity: 0, scale: 0.95 })}
        ...
        <div className="relative lg:absolute lg:inset-0 flex flex-col justify-center items-center px-6 mt-32 mb-20 lg:my-0 lg:pointer-events-none z-40">
          <div
            ref={textLayerRef}
