...
export default function Section03TechDetails() {
  ...
  const handleCardClick = (index: number) => {
    ...
    if (window.innerWidth < 1024) {
      const container = document.querySelector(".carousel-cover-flow-sec03");
      if (container) {
        const card = container.querySelector("li");
        const cardWidth = card ? card.clientWidth : 320;
        const gap = 24;
      ...
      <div className="max-w-7xl mx-auto w-full relative z-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-stretch" id="tech-blueprint-layout">
          ...
          <div className="lg:col-span-5 flex flex-col justify-between">
            {isMobile ? (
              <div className="flex flex-col gap-6 w-full mt-6" id="tech-blueprint-layout-mobile">
                <div className="flex justify-between items-center mb-2 px-2" id="mobile-carousel-indicator-sec03">
                  <span className="text-xs font-mono text-neutral-500 uppercase tracking-wider">// Technologies</span>
                  <span className="text-sm font-technical font-medium text-cyan-400 bg-neutral-900/40 border border-neutral-800/60 px-3 py-1 rounded-full">
                    {(selectedPoint !== null ? selectedPoint : 0) + 1} / {points.length}
                  </span>
                </div>
                <BlossomCarousel 
                  as="ul"
                  onScroll={handleScroll}
                  className="carousel-cover-flow carousel-cover-flow-sec03 overflow-x-auto no-scrollbar scroll-smooth pb-6"
                >
                  {points.map((pt) => {
                    const Icon = pt.icon;
                    const isActive = selectedPoint === pt.id;
                    return (
                      <li key={pt.id} className="relative flex-shrink-0 snap-center w-[80vw] sm:w-[320px]">
                        <div className="slide">
                          <div 
                            onClick={() => handleCardClick(pt.id)}
                            style={{ 
                              borderColor: isActive ? pt.layerColor : 'rgba(38, 38, 38, 0.4)',
                              boxShadow: isActive ? `0 15px 40px -10px ${pt.layerColor}30` : 'none'
                            }}
                            className={`p-6 rounded-[24px] border bg-neutral-950/90 min-h-[300px] flex flex-col justify-between transition-all duration-300 w-full focus:outline-none`}
                          >
                            <div>
                              <div className="flex items-center gap-3 mb-4" style={{ color: pt.layerColor }}>
                              ...
                              <h3 className="text-xl font-technical text-white uppercase tracking-wide mb-3">
                                {pt.title}
                              </h3>
                              
                              <p className="text-sm font-light text-neutral-400 leading-relaxed font-sans mb-6">
                                {pt.desc}
                              </p>
                            </div>

                            <div className="border-t border-neutral-900 pt-4 space-y-2">
                              <span className="text-[9px] uppercase font-mono tracking-widest mb-1 block" style={{ color: pt.layerColor }}>
                                technical_payload.hex
                              </span>
                              <div className="grid grid-cols-1 gap-1">
                                {pt.metrics.map((met, idx) => (
                                  <div key={idx} className="flex items-center gap-2 text-[11px] font-mono text-neutral-300">
                                    <span style={{ color: pt.layerColor }}>›</span>
                                    <span>{met}</span>
                                  </div>
                                ))}
                              </div>
